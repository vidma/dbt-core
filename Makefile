...
lint: .env ## Runs all code checks in parallel.
	$(DOCKER_CMD) pre-commit run flake8; $(DOCKER_CMD) pre-commit run mypy; $(DOCKER_CMD) pre-commit run black
...
test: .env ## Runs unit tests with py38 and code checks in parallel.
	$(DOCKER_CMD) tox -p -e py38; $(DOCKER_CMD) pre-commit run flake8; $(DOCKER_CMD) pre-commit run mypy
